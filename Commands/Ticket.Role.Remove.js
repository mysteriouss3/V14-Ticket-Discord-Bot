...
exports.run = async (client, message, args) => {
    ...
    WhitelistButton.setCustomId("WhitelistButtonRemove");
    ...
    WhitelistButton.setCustomId("OfficialButtonRemove");
    WhitelistButton.setEmoji("<:Key:1104383269149093968>");
    WhitelistButton.setLabel("・Yetkili Şikayet Menüsü");
    WhitelistButton.setStyle(Discord.ButtonStyle.Primary);
    
    const GeneralButton = new Discord.ButtonBuilder();

    WhitelistButton.setCustomId("GeneralButtonRemove");
    WhitelistButton.setEmoji("<:Key:1104383269149093968>");
    WhitelistButton.setLabel("Genel Sorun Menüsü");
    WhitelistButton.setStyle(Discord.ButtonStyle.Primary);

    const RoleRemove = new Discord.ActionRowBuilder().addComponents(WhitelistButton, OfficialButton, GeneralButton);
    ...
    if (Whitelist) {
        ...
        \` > \`・Bu menüden sadece rol ekleyebilirsiniz! Rol kaldırmak için! __Örn:__ \`${Prefix}ticket-rol-ekle\`
        \` > \`・Güvenilir Listeden kaldırılan roller artık hiç bir komutu kullanılmaz (1 Rol Eklenebilir)!
        \` > \`・Yetkili Şikayet'den kaldırılan roller artık hiç bir komutu kullanılmaz (3 Rol Eklenebilir)!
        \` > \`・Genel Sorun'dan kaldırılan roller artık hiç bir komutu kullanılmaz (3 Rol Eklenebilir)!`);


        message.react(Config.Embed_Info.SuccessEmote)
        message.reply({ embeds: [SuccessWarning], components: [RoleRemove] }).then( m => {
