...
const db = require('croxydb');
 ...
 const GeneralRemoveModal = new Discord.ModalBuilder();

 GeneralRemoveModal.setCustomId("GeneralRemoveModal");
 GeneralRemoveModal.setTitle('General Role Remove');

const GeneralContent = new Discord.TextInputBuilder();

GeneralContent.setCustomId("RoleID");
GeneralContent.setLabel('Lütfen bir rol idsi girin!');
GeneralContent.setMinLength(5);
GeneralContent.setStyle(Discord.TextInputStyle.Paragraph);
GeneralContent.setRequired(true);

const StandartAnnounceRow = new Discord.ActionRowBuilder().addComponents(GeneralContent);

GeneralRemoveModal.addComponents(StandartAnnounceRow);

client.on("interactionCreate", async (interaction) => {

    if (!interaction.isButton()) return;
    if (interaction.customId == "GeneralButtonRemove") {
        ...
        if (interaction.user.id == ButtonAuthor) {
            ...
            await interaction.showModal(GeneralRemoveModal);
...
client.on("interactionCreate", async (interaction) => {
    ...
    if (interaction.customId === 'GeneralRemoveModal') {
        ...
        db.unpush(`Guild_${interaction.guild.id}.Role.General_Report_Role`, RoleID );
        ...
        SuccessWarning.setDescription(`${Config.Embed_Info.SuccessEmote} | Hey! <@${interaction.user.id}>, Başarıyla "<@&${RoleID}>" adlı rol Genel Soru / Sorun rolünden kaldırıldı!`);
        ...
        if (LogType) {
            ...
            LogMessage.setDescription(`${Config.Embed_Info.WarningEmote} | Hey! <@${interaction.user.id}>, adlı yetkili tarafından bir Genel Soru / Sorun rolü kaldırıldı! İşte Detaylar!`);
            ...
            LogMessage.setAuthor({ name: `${interaction.guild.name}`, iconURL: `${interaction.guild.iconURL({ dynamic: true })}`});
            LogMessage.setColor(Config.Embed_Info.WarningColor);
            LogMessage.setDescription(`**Bir Genel Soru / Sorun Rolü Kaldırıldı!**
