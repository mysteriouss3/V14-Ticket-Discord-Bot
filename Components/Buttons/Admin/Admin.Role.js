...
const AdminAddModal = new Discord.ModalBuilder();

AdminAddModal.setCustomId("AdminAddModal");
AdminAddModal.setTitle('Admin Role Add');
...
AdminAddModal.addComponents(StandartAnnounceRow);

client.on("interactionCreate", async (interaction) => {

    if (!interaction.isButton()) return;
    if (interaction.customId == "OfficialButtonAdd") {
        ...
        if (interaction.user.id == ButtonAuthor) {
            ...
            await interaction.showModal(AdminAddModal);
...
client.on("interactionCreate", async (interaction) => {
    ...
    if (interaction.customId === 'AdminAddModal') {
        ...
        db.push(`Guild_${interaction.guild.id}.Role.Admin_Report_Role`, RoleID );
        ...
        SuccessWarning.setDescription(`${Config.Embed_Info.SuccessEmote} | Hey! <@${interaction.user.id}>, Başarıyla Yetkili Şikayet rolü "<@&${RoleID}>" adlı rol olarak ayarlandı!`);
        ...
        if (LogType) {
            ...
            LogMessage.setDescription(`${Config.Embed_Info.WarningEmote} | Hey! <@${interaction.user.id}>, adlı yetkili tarafından Yetkili Şikayet rolü eklendi! İşte Detaylar!`);
            ...
            LogMessage.setAuthor({ name: `${interaction.guild.name}`, iconURL: `${interaction.guild.iconURL({ dynamic: true })}`});
            LogMessage.setColor(Config.Embed_Info.WarningColor);
            LogMessage.setDescription(`**Bir Yetkili Şikayet Rolü Eklendi!**
