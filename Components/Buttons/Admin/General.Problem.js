...
const GeneralAddModal = new Discord.ModalBuilder();

GeneralAddModal.setCustomId("GeneralAddModal");
GeneralAddModal.setTitle('General Role Add');
...
GeneralAddModal.addComponents(StandartAnnounceRow);

client.on("interactionCreate", async (interaction) => {

    if (!interaction.isButton()) return;
    if (interaction.customId == "GeneralButtonAdd") {
        ...
        if (interaction.user.id == ButtonAuthor) {
            ...
            await interaction.showModal(GeneralAddModal);
...
client.on("interactionCreate", async (interaction) => {
    ...
    if (interaction.customId === 'GeneralAddModal') {
        ...
        db.push(`Guild_${interaction.guild.id}.Role.General_Report_Role`, RoleID );
        ...
        SuccessWarning.setDescription(`${Config.Embed_Info.SuccessEmote} | Hey! <@${interaction.user.id}>, Başarıyla Genel Soru / Sorun rolü "<@&${RoleID}>" adlı rol olarak ayarlandı!`);
        ...
        if (LogType) {
            ...
            LogMessage.setDescription(`${Config.Embed_Info.WarningEmote} | Hey! <@${interaction.user.id}>, adlı yetkili tarafından Genel Soru / Sorun rolü eklendi! İşte Detaylar!`);
            ...
            LogMessage.setAuthor({ name: `${interaction.guild.name}`, iconURL: `${interaction.guild.iconURL({ dynamic: true })}`});
            LogMessage.setColor(Config.Embed_Info.WarningColor);
            LogMessage.setDescription(`**Bir Genel Soru / Sorun Rolü Eklendi!**
