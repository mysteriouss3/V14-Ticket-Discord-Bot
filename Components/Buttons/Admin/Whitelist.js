...
const WhitelistAddModal = new Discord.ModalBuilder();

WhitelistAddModal.setCustomId("WhitelistAddModal");
WhitelistAddModal.setTitle('Whitelist Role Add');
...
WhitelistAddModal.addComponents(StandartAnnounceRow);

client.on("interactionCreate", async (interaction) => {

    if (!interaction.isButton()) return;
    if (interaction.customId == "WhitelistButtonAdd") {
        ...
        if (interaction.user.id == ButtonAuthor) {
            ...
            await interaction.showModal(WhitelistAddModal);
...
client.on("interactionCreate", async (interaction) => {
    ...
    if (interaction.customId === 'WhitelistAddModal') {
        ...
        db.push(`Guild_${interaction.guild.id}.Role.Whitelist_Role`, RoleID );
        ...
        SuccessWarning.setDescription(`${Config.Embed_Info.SuccessEmote} | Hey! <@${interaction.user.id}>, Başarıyla whitelist rolü "<@&${RoleID}>" adlı rol olarak ayarlandı!`);
        ...
        if (LogType) {
            ...
            LogMessage.setDescription(`${Config.Embed_Info.WarningEmote} | Hey! <@${interaction.user.id}>, adlı yetkili tarafından Whitelist rolü eklendi! İşte Detaylar!`);
            ...
            LogMessage.setAuthor({ name: `${interaction.guild.name}`, iconURL: `${interaction.guild.iconURL({ dynamic: true })}`});
            LogMessage.setColor(Config.Embed_Info.WarningColor);
            LogMessage.setDescription(`**Whitelist Rolü Eklendi!**
