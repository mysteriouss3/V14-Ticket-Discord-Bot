...
const db = require('croxydb');
 ...
 const AdminRemoveModal = new Discord.ModalBuilder();

 AdminRemoveModal.setCustomId("AdminRemoveModal");
 AdminRemoveModal.setTitle('Admin Role Remove');
...
AdminRemoveModal.addComponents(StandartAnnounceRow);

client.on("interactionCreate", async (interaction) => {

    if (!interaction.isButton()) return;
    if (interaction.customId == "OfficialButtonRemove") {
        ...
        if (interaction.user.id == ButtonAuthor) {
            ...
            await interaction.showModal(AdminRemoveModal);
...
client.on("interactionCreate", async (interaction) => {
    ...
    if (interaction.customId === 'AdminRemoveModal') {
        ...
        db.unpush(`Guild_${interaction.guild.id}.Role.Admin_Report_Role`, RoleID );
        ...
        SuccessWarning.setDescription(`${Config.Embed_Info.SuccessEmote} | Hey! <@${interaction.user.id}>, Başarıyla "<@&${RoleID}>" adlı rol Yetkili Şikayet rolünden kaldırıldı!`);
        ...
        if (LogType) {
            ...
            LogMessage.setDescription(`${Config.Embed_Info.WarningEmote} | Hey! <@${interaction.user.id}>, adlı yetkili tarafından bir Yetkili Şikayet rolü kaldırıldı! İşte Detaylar!`);
            ...
            LogMessage.setAuthor({ name: `${interaction.guild.name}`, iconURL: `${interaction.guild.iconURL({ dynamic: true })}`});
            LogMessage.setColor(Config.Embed_Info.WarningColor);
            LogMessage.setDescription(`**Bir Yetkili Şikayet Rolü Kaldırıldı!**
